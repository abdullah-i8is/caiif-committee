      ...
      <Row gutter={[24, 0]}>
        <Col span={24} md={24} className="header-control">
          <Badge size="default" count={0}>
            ...
            {/* <Text style={{ fontSize: 20, fontWeight: "700", margin: "0 0 0 20px" }}>{loginUser.user}, {Number(loginUser.role) === 0 ? "Admin" : Number(loginUser.role) === 1 ? "Managment" : Number(loginUser.role) === 2 ? "H.O.D" : "Travler"}</Text>
            <Dropdown menu={menuProps} trigger={["hover"]}>
              <Avatar shape="circle" src={avtar} size={50} />
            </Dropdown> */}
          </Badge>
        </Col>
      </Row>
