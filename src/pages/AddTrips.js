import { CommentOutlined, PlusOutlined, UserAddOutlined, HeatMapOutlined, CarOutlined, ArrowRightOutlined, CloseOutlined, DeleteOutlined } from "@ant-design/icons";
import {
    Typography,
    Tooltip,
    FloatButton,
...
function AddEmployess() {
    ...
    useEffect(() => {
        if (editEmployee) {
            ...
            if (find) {
                ...
                form.setFieldsValue({
                    ...find,
                    dateofbirth: dayjs(find.dateofbirth),//find.dateofbirth
                })
    ...
    const getEmployess = () => {
        ...
        onSnapshot(colRef2, (snapshot) => {
            const employess = snapshot.docs.map((doc) => {
                ...
                return {
                    ...doc.data(),
                    dateofbirth: data?.dateofbirth ? data?.dateofbirth?.toDate() : null,//new Date(data.dateofbirth.toDa),
                    passportExpiry: data?.passportExpiry ? data?.passportExpiry?.toDate() : null,//new Date(data.dateofbirth.toDa),
                    id: doc.id,
                }
            })
            setData(employess)
    ...
    const columns = [
        {
            ...
            render: (text, record) => {
                return (
                    <>
                        <Checkbox disabled={!record.status} onChange={(e) => handleAddTripUsers(e, record)} />
                    </>
                )
            },
        },
        {
            ...
            key: 'unitNo',
            render: (text, record) => {
                return unitNo.find((f, i) => f.id === record.unitNo).unitNo
            }
        },
        {
            title: 'Name',
    ...
    return (
        <>
            ...
            <Title level={3}>ADD TRIPS</Title>
            <Table dataSource={data.filter(f => f.department === loginUser.department)} columns={columns} />
            <Modal
                centered
                width="50%"
                title={editEmployee !== null ? "EDIT EMPLOYEE" : "ADD EMPLOYEE"}
                onOk={handleOk}
                ...
                {editEmployee ? [<Button key="back" onClick={() => {
                    setEditEmployee(null)
                    onReset()
                    setShowModal(false)
                }}>
                    ...
                    <Row gutter={[24, 0]}>
                        ...
                        <Col xs={24} sm={24} md={8} lg={8} xl={8}>
                            <Form.Item
                                ...
                                <DatePicker dia style={{ width: "100%" }} />
                            </Form.Item>
                        </Col>
                        <Col xs={24} sm={24} md={8} lg={8} xl={8}>
                            <Form.Item
            ...
            {tripUsers && tripUsers.length > 0 || tripStarted ? (
                <FloatButton.Group
                    shape="circle"
                    trigger="hover"
                    type="primary"
                        ...
                        <FloatButton
                            ...
                            onClick={() => {
                                if (tripUsers && tripUsers.length > 0) {
                                    navigate({ pathname: `/add-trip-detail` }, { state: { tripUsers: tripUsers } })
                                } else {
                                    api.error({
                                        message: "Error",
                                        description: "Please Select One or More Traveler"
                                    })
                                }
                            }} />
                    </Tooltip>
                ...
                >
                    <Tooltip title="Add users" placement="left">
                        <FloatButton icon={<UserAddOutlined />} onClick={handleOpenModal} />
                    </Tooltip>
                    <Tooltip title="Add trips" placement="left">
                        <FloatButton icon={<CarOutlined />} onClick={() => setTripStarted(true)} />
                    </Tooltip>
                </FloatButton.Group>
            )}
        </>
